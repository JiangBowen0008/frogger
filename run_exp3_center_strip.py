#!/usr/bin/env python3
"""Exp 3: Run with center-strip full-depth palm collision."""
...
opt = BatchedGraspOptimizer(sdf, num_envs=500, device="cuda", hand="rh", hand_type="leap",
                            palm_contact=True)  # palm contact ON, collision is center-strip
results = opt.optimize(actuation_targets=[(act_pos, None)], object_center=obj_center,
                       steps=600, lr=0.005,
                       save_path="output/grasps/compare_exp3_center_strip.pt")

# Run collision overlay on result
print("\nRunning collision overlay on Exp 3 result...")
